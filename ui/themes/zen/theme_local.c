...
void quickbar(lv_obj_t *par)
{
    ...
    for (int i = 0; i < 4; i++) {
        ...
        hub_ico_add(b, items[i].ico, p->accent, 14); /* FE Lucide 14×14 */
        lv_obj_t *l = lv_label_create(b);
        lv_label_set_text(l, hub_model()->settings.lang_zh ? items[i].zh : items[i].en);
...
void theme_card_chrome(lv_obj_t *obj, bool active)
{
    const hub_palette_t *p = hub_palette();
    (void)active;
    /* FE zen: radius 0, hairline, no shadow */
