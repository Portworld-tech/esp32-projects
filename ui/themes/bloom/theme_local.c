/** Bloom — helpers / chrome / callbacks */
...
void quickbar(lv_obj_t *par)
{
    ...
    lv_obj_set_height(bar, 36); /* FE HubQuickBar dense */
    ...
    for (int i = 0; i < 4; i++) {
        ...
        lv_obj_remove_style_all(b);
        /* FE: radius-sm, soft card, accent icon */
        lv_obj_set_style_radius(b, p->radius_sm, 0);
        lv_obj_set_style_bg_opa(b, LV_OPA_COVER, 0);
        lv_obj_set_style_bg_color(b, p->bg_card, 0);
        ...
        hub_ico_add(b, items[i].ico, p->accent, 14);
        lv_obj_t *l = lbl(b, zh ? items[i].zh : items[i].en, p->t2, hub_font());
...
void theme_card_chrome(lv_obj_t *obj, bool active)
{
    const hub_palette_t *p = hub_palette();
    (void)active;
    /* Soft bloom cards — radius 28, gentle rose shadow (no hard bevel) */
